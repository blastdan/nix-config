...
with lib.blastdan; let
  cfg = config.blastdan.desktop.addons.gtk;
  gdmCfg = config.services.xserver.displayManager.gdm;
in {
  options.blastdan.desktop.addons.gtk = with types; {
    ...
    cursor = {
      ...
      pkg = mkOpt package pkgs.blastdan.bibata-cursors "The package to use for the cursor theme.";
    };
    icon = {
      name =
        mkOpt str "Papirus"

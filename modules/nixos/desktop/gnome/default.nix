...
with lib.blastdan; let
  cfg = config.blastdan.desktop.gnome;
...
in {
  options.blastdan.desktop.gnome = with types; {
    enable =
      mkBoolOpt false "Whether or not to use Gnome as the desktop environment.";
    wallpaper = {
      light = mkOpt (oneOf [str package]) pkgs.blastdan.wallpapers.atmosphere "The light wallpaper to use.";
      dark = mkOpt (oneOf [str package]) pkgs.blastdan.wallpapers.atmosphere "The dark wallpaper to use.";
    };
  ...
  config = mkIf cfg.enable {
    blastdan.system.xkb.enable = true;
    blastdan.desktop.addons = {
      gtk = enabled;
      wallpapers = enabled;
    ...
    environment.systemPackages = with pkgs;
      [
        (hiPrio blastdan.xdg-open-with-portal)
        wl-clipboard
        gnome.gnome-tweaks
    ...
    systemd.services.blastdan-user-icon = {
      ...
      script = ''
        config_file=/var/lib/AccountsService/users/${config.blastdan.user.name}
        icon_file=/run/current-system/sw/share/blastdan-icons/user/${config.blastdan.user.name}/${config.blastdan.user.icon.fileName}
    ...
    blastdan.home.extraOptions = {
      dconf.settings = let
        user = config.users.users.${config.blastdan.user.name};
        ...
        nested-default-attrs {
          "org/gnome/shell" = {
            ...
            favorite-apps =
              ...
              ++ optional config.blastdan.apps.firefox.enable "firefox.desktop"
              ++ optional config.blastdan.apps.vscode.enable "code.desktop"
              ++ optional config.blastdan.desktop.addons.foot.enable "foot.desktop";
          };
          ...
          "org/gnome/shell/extensions/Logo-menu" = {
            ...
            menu-button-terminal =
              if config.blastdan.desktop.addons.term.enable
              then lib.getExe config.blastdan.desktop.addons.term.pkg
